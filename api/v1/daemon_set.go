...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all DaemonSets by agent name
// @Tags DaemonSet
// @Produce json
...
// @Router /api/v1/daemon-sets [GET]
func (d daemonSetApi) Get(context echo.Context) error {
	agent := context.QueryParam("agent")
	option := GetQueryOption(context)
	data, total := d.daemonSetService.Get(agent, option)
...
// @Forbidden 403 {object} common.ResponseDTO
// @Failure 400 {object} common.ResponseDTO
// @Router /api/v1/daemon-sets/{owner-reference} [GET]
func (d daemonSetApi) GetByOwnerReference(context echo.Context) error {
	agent := context.QueryParam("agent")
	option := GetQueryOption(context)
	ownerReference := context.Param("owner-reference")
	data, total := d.daemonSetService.GetByOwnerReference(agent, ownerReference, option)
	metadata := common.GetPaginationMetadata(option.Pagination.Page, option.Pagination.Limit, total, int64(len(data)))
	uri := strings.Split(context.Request().RequestURI, "?")[0]
	if option.Pagination.Page > 0 {
		metadata.Links = append(metadata.Links, map[string]string{"prev": uri + "?order=" + context.QueryParam("order") + "&page=" + strconv.FormatInt(option.Pagination.Page-1, 10) + "&limit=" + strconv.FormatInt(option.Pagination.Limit, 10)})
...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all DaemonSets by agent name and process id
// @Tags DaemonSet
// @Produce json
...
// @Forbidden 403 {object} common.ResponseDTO
// @Failure 400 {object} common.ResponseDTO
// @Router /api/v1/daemon-sets/{processId} [GET]
func (d daemonSetApi) GetByProcessId(context echo.Context) error {
	agent := context.QueryParam("agent")
	option := GetQueryOption(context)
	processId := context.Param("processId")
	data, total := d.daemonSetService.GetByProcessId(agent, processId, option)
	metadata := common.GetPaginationMetadata(option.Pagination.Page, option.Pagination.Limit, total, int64(len(data)))
	uri := strings.Split(context.Request().RequestURI, "?")[0]
	if option.Pagination.Page > 0 {
		metadata.Links = append(metadata.Links, map[string]string{"prev": uri + "?order=" + context.QueryParam("order") + "&page=" + strconv.FormatInt(option.Pagination.Page-1, 10) + "&limit=" + strconv.FormatInt(option.Pagination.Limit, 10)})
