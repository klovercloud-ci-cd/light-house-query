...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all ConfigMaps by agent name
// @Tags ConfigMap
// @Produce json
...
// @Success 200 {object} common.ResponseDTO{data=[]v1.ConfigMap{}}
...
func (c configMapApi) Get(context echo.Context) error {
	...
	data, total := c.configMapService.Get(agent, option)
...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all ConfigMaps by agent name and owner reference
// @Tags ConfigMap
// @Produce json
...
// @Forbidden 403 {object} common.ResponseDTO
// @Failure 400 {object} common.ResponseDTO
// @Router /api/v1/config-maps/{owner-reference} [GET]
func (c configMapApi) GetByOwnerReference(context echo.Context) error {
	agent := context.QueryParam("agent")
	option := GetQueryOption(context)
	ownerReference := context.Param("owner-reference")
	data, total := c.configMapService.GetByOwnerReference(agent, ownerReference, option)
	metadata := common.GetPaginationMetadata(option.Pagination.Page, option.Pagination.Limit, total, int64(len(data)))
	uri := strings.Split(context.Request().RequestURI, "?")[0]
	if option.Pagination.Page > 0 {
		metadata.Links = append(metadata.Links, map[string]string{"prev": uri + "?order=" + context.QueryParam("order") + "&page=" + strconv.FormatInt(option.Pagination.Page-1, 10) + "&limit=" + strconv.FormatInt(option.Pagination.Limit, 10)})
...
func NewConfigMapApi(configMapService service.ConfigMap) api.ConfigMap {
	return &configMapApi{
