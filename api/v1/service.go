...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all services by agent name
// @Tags Service
// @Produce json
...
// @Success 200 {object} common.ResponseDTO{data=[]v1.Service{}}
// @Forbidden 403 {object} common.ResponseDTO
// @Failure 400 {object} common.ResponseDTO
// @Router /api/v1/services [GET]
func (s serviceApi) Get(context echo.Context) error {
	agent := context.QueryParam("agent")
	option := GetQueryOption(context)
	data, total := s.serviceService.Get(agent, option)
...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all Services by agent name and owner reference
// @Tags ServiceAccount
// @Produce json
...
// @Forbidden 403 {object} common.ResponseDTO
// @Failure 400 {object} common.ResponseDTO
// @Router /api/v1/services/{owner-reference} [GET]
func (s serviceApi) GetByOwnerReference(context echo.Context) error {
	agent := context.QueryParam("agent")
	option := GetQueryOption(context)
	ownerReference := context.Param("owner-reference")
	data, total := s.serviceService.GetByOwnerReference(agent, ownerReference, option)
	metadata := common.GetPaginationMetadata(option.Pagination.Page, option.Pagination.Limit, total, int64(len(data)))
	uri := strings.Split(context.Request().RequestURI, "?")[0]
	if option.Pagination.Page > 0 {
		metadata.Links = append(metadata.Links, map[string]string{"prev": uri + "?order=" + context.QueryParam("order") + "&page=" + strconv.FormatInt(option.Pagination.Page-1, 10) + "&limit=" + strconv.FormatInt(option.Pagination.Limit, 10)})
...
// NewServiceApi returns api.Service type api
func NewServiceApi(serviceService service.Service) api.Service {
	return &serviceApi{
