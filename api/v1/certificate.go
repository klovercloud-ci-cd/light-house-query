...
// @Description Api for getting a Certificate by id, agent name, and process id
// @Tags Certificate
// @Produce json
...
func (c certificateApi) GetByID(context echo.Context) error {
	id := context.Param("id")
	agent := context.QueryParam("agent")
	processId := context.QueryParam("processId")
	data := c.certificateService.GetById(id, agent, processId)
