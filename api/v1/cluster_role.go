...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all Cluster Roles by agent name
// @Tags ClusterRole
// @Produce json
...
func (c clusterRoleApi) Get(context echo.Context) error {
	...
	data, total := c.clusterRoleService.Get(agent, option)
...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all Cluster Roles by agent name and owner reference
// @Tags ClusterRole
// @Produce json
...
// @Forbidden 403 {object} common.ResponseDTO
// @Failure 400 {object} common.ResponseDTO
// @Router /api/v1/cluster-roles/{owner-reference} [GET]
func (c clusterRoleApi) GetByOwnerReference(context echo.Context) error {
	agent := context.QueryParam("agent")
	option := GetQueryOption(context)
	ownerReference := context.Param("owner-reference")
	data, total := c.clusterRoleService.GetByOwnerReference(agent, ownerReference, option)
	metadata := common.GetPaginationMetadata(option.Pagination.Page, option.Pagination.Limit, total, int64(len(data)))
	uri := strings.Split(context.Request().RequestURI, "?")[0]
	if option.Pagination.Page > 0 {
		metadata.Links = append(metadata.Links, map[string]string{"prev": uri + "?order=" + context.QueryParam("order") + "&page=" + strconv.FormatInt(option.Pagination.Page-1, 10) + "&limit=" + strconv.FormatInt(option.Pagination.Limit, 10)})
...
// Get... Get Api
// @Summary Get api
// @Description Api for getiing all Cluster Roles by agent name and process id
// @Tags ClusterRole
// @Produce json
...
// @Forbidden 403 {object} common.ResponseDTO
// @Failure 400 {object} common.ResponseDTO
// @Router /api/v1/cluster-roles/{processId} [GET]
func (c clusterRoleApi) GetByProcessId(context echo.Context) error {
	agent := context.QueryParam("agent")
	option := GetQueryOption(context)
	processId := context.Param("processId")
	data, total := c.clusterRoleService.GetByOwnerReference(agent, processId, option)
	metadata := common.GetPaginationMetadata(option.Pagination.Page, option.Pagination.Limit, total, int64(len(data)))
	uri := strings.Split(context.Request().RequestURI, "?")[0]
	if option.Pagination.Page > 0 {
		metadata.Links = append(metadata.Links, map[string]string{"prev": uri + "?order=" + context.QueryParam("order") + "&page=" + strconv.FormatInt(option.Pagination.Page-1, 10) + "&limit=" + strconv.FormatInt(option.Pagination.Limit, 10)})
